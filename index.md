...
<div class="gapc-wrap"><div class="gapc-head"><span class="gapc-title">💱 면세점 vs 국내, 어디가 더 쌀까</span><span class="gapc-when">{{ _g.carousel_date }} 기준</span></div><a class="gapc" href="{{ _g.url | relative_url }}"><ul class="gapc-track">{% for it in _g.carousel %}<li class="gapc-item"><span class="gapc-txt">{{ it }}</span></li>{% endfor %}</ul><span class="gapc-more">면세 vs 국내 전체 비교 보기 →</span></a></div>
